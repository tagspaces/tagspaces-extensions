...
const { existsSync, readdirSync, mkdirSync, statSync, copyFileSync, writeFileSync } = require('fs');
...
if (existsSync(patchesDir)) {
  const patchFiles = readdirSync(patchesDir).filter(file => file.endsWith('.patch'));
  ...
  console.log(`No patches directory found at ${patchesDir} – skipping patching.`);
}
...
try {
  copyFileSync(join(pdfjsDir, 'build', 'version.json'), join(destDir, 'version.json'));
} catch (error) {
